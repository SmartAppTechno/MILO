...
<div class="row wrapper border-bottom white-bg page-heading">
	<div class="col-lg-10">
		<h2>Vídeos de la Página</h2>
		<form role="form" action="<?php echo base_url('nuevo_video_pagina'); ?>" method="post">
				...
				<tbody>
					<?php
						foreach($videos as $video){
							...
							echo '<td>' . $video['orden'] . '</td>';
							echo '<td class="acciones"><form role="form" action="'.base_url('editar_video_pagina').'" method="post">
			                        <input type="hidden" name="id" value="'.$video['id'].'">
			                        <button type="submit" class="btn btn-w-m btn-primary"><i class="fa fa-pencil"></i></button>
			                    </form>
			                    <form role="form" action="'.base_url('eliminar_video_pagina').'" method="post">
			                        <input type="hidden" name="id" value="'.$video['id'].'">
