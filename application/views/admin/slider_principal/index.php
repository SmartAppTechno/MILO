...
<div class="row wrapper border-bottom white-bg page-heading">
	<div class="col-lg-10">
		...
		<form role="form" action="<?php echo base_url('nueva_foto'); ?>" method="post">
            <button type="submit" class="btn btn-w-m btn-primary">Nueva Foto</button>
        </form>
	</div>
</div>
				...
				<tbody>
					<?php
						foreach($fotos as $foto){
							...
							echo '<td class="acciones"><form role="form" action="'.base_url('editar_foto').'" method="post">
			                        <input type="hidden" name="id" value="'.$foto['id'].'">
			                        <button type="submit" class="btn btn-w-m btn-primary"><i class="fa fa-pencil"></i></button>
			                    </form>
			                    <form role="form" action="'.base_url('eliminar_foto').'" method="post">
			                        <input type="hidden" name="id" value="'.$foto['id'].'">
